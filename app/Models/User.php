...
class User extends Authenticatable
{
    ...
    public function register($full_name,$email_id,$password,$user_ip,$location,$gender)
    {
        try {
    ...
    public function lastLogin($email_id,$user_ip): bool
    {
        try {
            $result = $this->where('email',$email_id)
                            ->update(['last_login_at' => \Carbon\Carbon::now('Asia/Karachi'),'last_login_ip'=>$user_ip]);
            if ($result){
                return true;
            }
            return false;
    ...
    public function getProfileData($email_id){
        try {
            $result = $this->select('name','email','location','gender')->where('email',$email_id)->get();
            if (count($result)){
                return $result;
            }
            return null;
